...
execute unless score $GameState md_state matches 3 run return 1
...
execute as @e[distance=..1,type=marker,tag=md_target,tag=md_selected] if score @s md_score >= $AddTargetThreshold md_config run scoreboard players add #temp md_state 1
