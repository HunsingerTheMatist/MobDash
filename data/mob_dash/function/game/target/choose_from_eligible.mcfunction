...
execute as @e[distance=..1,type=marker,tag=md_eligible] run scoreboard players operation $MaxChoice md_state += @s md_weight

# Randomize a number from [0-$MaxChoice)
...
execute as @e[distance=..1,type=marker,tag=md_eligible] run function mob_dash:game/target/get_mob_from_choice
