...
scoreboard players operation @e[distance=..1,type=marker,tag=md_target,tag=md_selected] md_score < $MaxTargetScore md_config

# Change the string 'point' to 'points' since the score is now above 1
execute if score $MaxTargetScore md_config matches 2.. as @e[distance=..1,type=marker,tag=md_target,tag=md_selected] run data modify entity @s data.PointString set value " points"
