...
execute unless score $ActionBarCache md_state matches 1 as @e[distance=0,type=marker,tag=md_selected] if score @s md_index = #temp2 md_state run function mob_dash:game/build_action_bar
scoreboard players set $ActionBarCache md_state 1
tag @e[distance=0,type=marker,tag=md_processed] remove md_processed
