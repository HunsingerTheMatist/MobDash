...
execute unless dimension mob_dash:md_markers in mob_dash:md_markers positioned 0 0 0 run return run function mob_dash:menu/teams/randomize/start

# Pick which team colors to use (prioritizing currently-occupied teams)
function mob_dash:menu/teams/randomize/pick_teams

execute store result score #team_idx md_state run random value 0..7

scoreboard players set #silent_joins md_state 1
execute if entity @n[distance=..1,type=marker,tag=md_chosen] as @a[sort=random] run function mob_dash:menu/teams/randomize/step
