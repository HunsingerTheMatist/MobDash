...
execute unless dimension mob_dash:mb_markers in mob_dash:mb_markers positioned 0 0 0 run return run function mob_dash:menu/display_teams_menu
...
tellraw @s [{text:"- "}, {text:"[", color:gold, bold: true}, {text:"Join ", color:white, click_event: {action:run_command, command:"trigger SetTeam set 9"}, extra: [{selector:"@n[distance=..1,type=marker,tag=md_team,scores={md_team=9}]"}]}, {text:"]", color:gold, bold: true}]
