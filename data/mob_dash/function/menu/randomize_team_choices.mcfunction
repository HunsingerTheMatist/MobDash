...
execute as @e[distance=..1,type=marker,tag=md_team,scores={md_team=1..8},sort=random] at @a if score @s md_team = @p md_team \ 
    unless entity @s[tag=md_chosen] if score #picks_left md_state matches 1.. run function mob_dash:menu/randomize_choose_team

# Pass 2: fill remaining slots from un-chosen teams (random)
execute as @e[distance=..1,type=marker,tag=md_team,scores={md_team=1..8},tag=!md_chosen,sort=random] if score #picks_left md_state matches 1.. run function mob_dash:menu/randomize_choose_team
